...
// 추가 (핵심: 첫 신호에서 Ready 해제)
    QObject::connect(Service_inter.get(), &IC_interStubImpl::signalGear_inter,
                 gearPtr.get(),
                 [gearPtr](const std::string& g){
                     static bool started = false;
                     if (!started) {
                         started = true;
                         gearPtr->clientConnectedSignal(); // Ready -> 주행 화면 전환
                     }
                 });
...
/*////////////////////////////////////////////////// team7 code start*/
    ...
    if (receiver.initialize() == Receiver::SUCCEDED) {
        receiver.start();
    } else {
        qCritical() << "CAN receiver failed to initialize. Check can0 interface.";
    }
